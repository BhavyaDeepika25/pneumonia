...
st.markdown(f'<p class="glowing-text">PNEUMONIA DETECTION</p>', unsafe_allow_html=True)
...
if st.button('Classify') and user_image is not None:
	...
	all_keys=names_dict.keys()
	final_val=np.argmax(final_probs_list)
	if final_val in all_keys:
		final_pred_val=names_dict[final_val]
		st.image(user_image,use_column_width=True)
		st.markdown(f'''<h3 align='center'>Classified Class: {final_pred_val}</h3>''',unsafe_allow_html=True)
		st.balloons()
